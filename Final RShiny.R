  ...
  body = dashboardBody(
    tabItems(
      tabItem(tabName = "AttributesDesc",
              fluidPage(
                titlePanel("Heart Disease Predictions"),
                mainPanel(
                  tags$div(
      ...
      tabItem(tabName = "Analysis",
              fluidPage(
                ...
                fluidRow(
                  box(title = "Analysis Plot 7", status = "primary", solidHeader = TRUE, 
                      plotOutput("analysisPlot7")),
                  box(title = "Analysis Plot 8", status = "primary", solidHeader = TRUE, 
                      plotOutput("analysisPlot8"))
                )
              )
      ),
...
server <- function(input, output) {
  ...
  output$analysisPlot1 <- renderPlot({
    ggplot(df, aes( x = age)) +
      geom_histogram(alpha = 0.7, fill = "navy") +
      theme_minimal() +
  ...
  output$analysisPlot7 <- renderPlot({
    ggplot(df, aes(x = ST_slope, fill = target)) +
      geom_bar(position = "dodge") +
      theme_minimal() +
      labs(title = "Count of ST Slope by Heart Disease", x = "ST Slope", y = "Count", fill = "Heart Disease")
  })
  
  output$analysisPlot8 <- renderPlot({
    ggplot(df, aes(x = factor(resting_ecg), fill = factor(target))) +
      geom_bar(position = "dodge") +
      theme_minimal() +
      labs(title = "Count of Resting ECG by Heart Disease", x = "Resting ECG", y = "Count", fill = "Heart Disease")
  })
